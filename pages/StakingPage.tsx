...
const StakingPage: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.warn("⚡ Rig Telemetry error in UI, service fallbacks handle this.");
    }
  };
  ...
  const handleAction = async (action: 'stake' | 'claim') => {
    if (!state.account) {
      setStatus({ type: 'error', msg: 'Neural Link Required ⚡' });
      return;
    }

    if (action === 'stake' && (!stakeAmount || isNaN(Number(stakeAmount)) || Number(stakeAmount) <= 0)) {
      setStatus({ type: 'error', msg: 'Invalid Energy Volume.' });
      return;
    }

    const loadingKey = action === 'stake' ? 'staking' : 'claiming';
    setGlobalLoading(loadingKey, true);
    setStatus({ type: 'loading', msg: action === 'stake' ? `Charging Rig Core with MCB...` : 'Extracting MCB energy...' });
    ...
    try {
      ...
      setStatus({ 
        type: 'success', 
        msg: action === 'stake' ? `🎉 Rig Core Upgraded with MCB ✨` : `🎁 MCB Energy Extracted Successfully ✨` 
      });
      setStakeAmount('');
      setPercentage(0);
      ...
      <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 border-b border-white/5 pb-10">
          ...
          <div className="flex items-center gap-4">
             ...
             <div>
                <h1 className="text-5xl font-black tracking-tighter uppercase italic flex items-center gap-3 text-white">
                  Mining <span className="text-amber-500">Rig</span>
                </h1>
                <div className="flex items-center gap-3 mt-1">
                  ...
                  <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest font-mono">ID: MEEBOT-RIG-S4</span>
                </div>
             </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 flex-grow lg:max-w-3xl">
          {[
            ...
            { label: 'Grid APR', val: '24.5', unit: '%', color: 'text-emerald-400' },
          ].map((stat, i) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
              ...
              <div className="space-y-10">
                <div className="relative group">
                  ...
                  <div className="absolute right-10 top-1/2 -translate-y-1/2 flex flex-col items-end">
                    ...
                    <button onClick={() => handlePercentageChange(100)} className="text-[10px] text-amber-500 font-black mt-2 hover:text-white transition-colors">MAX CAP</button>
                  </div>
                </div>

                <div className="space-y-8 p-8 bg-black/40 border border-white/5 rounded-[2rem]">
                  <div className="flex justify-between items-center px-2">
                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Mining Load Level</span>
                ...
                >
                  {isProcessing ? 'SYNCING WITH CORE...' : 'START MINING RITUAL ⚡'}
                </button>
              </div>
            </div>
          </section>
        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          <div className="p-6 bg-white/5 rounded-[2rem] border border-white/5 text-center">
            <p className="text-[8px] font-black text-slate-600 uppercase tracking-[0.3em]">MeeBot_Protocol_v3.1_RigControl</p>
          </div>
        </div>
      </div>
    </div>
